...
export default function SectionAdminLaporan() {
    ...
    const fetchData = async () => {
        ...
        // 2. Ambil Config Thread dari Database
    ...
    const handleAction = async (id: string, status: 'APPROVED' | 'REJECTED') => {
        ...
        else { toast.success(`Status: ${status}`, { id: tId }); fetchData(); }
    };

    // --- ACTION: TRANSMIT TO DISCORD (AUTO THREAD MAPPING) ---
                ...
                {showConfig && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className={`bg-[#FFD100] ${boxBorder} ${hardShadow} rounded-[35px] p-8 space-y-6`}>
                            <div className="flex items-center gap-3"><Hash size={24} /><h3 className="font-[1000] italic uppercase tracking-tighter">Monthly Thread Control</h3></div>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                {Object.entries(threadConfigs).map(([key, value]) => (
                                    <div key={key} className="space-y-2">
                                        <label className="text-[9px] font-black uppercase opacity-60 ml-2">{key.replace('thread_', '').replace('_', ' ')}</label>
                                        <input value={value} onChange={(e) => setThreadConfigs({ ...threadConfigs, [key]: e.target.value })} className="w-full bg-white border-2 border-black p-3 rounded-xl font-black text-[10px] shadow-[3px_3px_0px_#000] outline-none" />
                                    </div>
                                ))}
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredData.map((lap) => (
                        <div key={lap.id} className={`bg-white ${boxBorder} ${hardShadow} rounded-[35px] overflow-hidden flex flex-col group relative`}>
                            {/* Individual Delete */}
                            <button onClick={() => setDeleteModal({ show: true, type: 'SINGLE', id: lap.id })} className="absolute top-2 right-2 z-20 bg-white/20 hover:bg-red-500 hover:text-white p-2 rounded-lg border-2 border-black opacity-0 group-hover:opacity-100 transition-all"><X size={14} /></button>
                            ...
                            <div className="p-6 flex-1 space-y-4">
                                <div className="bg-slate-50 border-2 border-black p-4 rounded-2xl h-40 overflow-y-auto scrollbar-hide text-[10px] font-bold whitespace-pre-wrap italic">
                                    {lap.isi_laporan}
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    {lap.status === 'PENDING' ? (
                                        <>
                                            <button onClick={() => handleAction(lap.id, 'REJECTED')} className="bg-[#FF4D4D] border-2 border-black py-2.5 rounded-xl font-black text-[10px] uppercase shadow-[3px_3px_0px_#000] active:translate-y-1 transition-all">Reject</button>
                                            <button onClick={() => handleAction(lap.id, 'APPROVED')} className="bg-[#A3E635] border-2 border-black py-2.5 rounded-xl font-black text-[10px] uppercase shadow-[3px_3px_0px_#000] active:translate-y-1 transition-all">Approve</button>
                                        </>
                                    ) : (
            ...
            <AnimatePresence>
                {deleteModal.show && (
                    <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className={`w-full max-w-sm bg-white ${boxBorder} ${hardShadow} rounded-[30px] p-6 space-y-4`}>
                            <div className="flex items-center gap-3 text-red-600"><AlertOctagon size={32} /><h3 className="font-black uppercase italic tracking-tighter text-xl text-slate-900">Confirm Delete</h3></div>
                            <p className="text-xs font-black uppercase italic opacity-60 text-slate-900">{deleteModal.type === 'ALL' ? "Bersihkan database laporan?" : "Hapus laporan ini permanen?"}</p>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button onClick={() => { setDeleteModal({ show: false, type: 'ALL' }); setConfirmInput(""); }} className="bg-slate-200 border-2 border-black py-2 rounded-xl font-black uppercase text-[10px] text-slate-950">Batal</button>
            ...
            <AnimatePresence>
                {previewData && (
                    <div className="fixed inset-0 z-[200] bg-black/90 p-4 flex items-center justify-center overflow-y-auto">
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white max-w-2xl w-full rounded-[40px] border-[6px] border-slate-950 shadow-[10px_10px_0px_#3B82F6] overflow-hidden">
                            ...
                            <div className="p-8 space-y-6">
                                ...
                                <div className="flex flex-col md:flex-row gap-4 pt-4 text-slate-950">
                                    <div className="flex-1 bg-slate-50 p-4 rounded-2xl border-2 border-black border-dashed">
                                        ...
                                        <p className="text-xs font-black truncate">{threadConfigs[previewData.jenis_laporan === 'KASUS BESAR' ? 'thread_kasus_besar' : `thread_${previewData.jenis_laporan.toLowerCase()}` as keyof typeof threadConfigs] || 'N/A'}</p>
                                    </div>
