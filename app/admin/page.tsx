...
import {
    ...
    ArrowLeft, Banknote, FileSpreadsheet, ClipboardList
} from 'lucide-react';
...
import SectionAdminLaporan from "./components/SectionAdminLaporan"; // SECTION BARU
...
export default function AdminPortal() {
    ...
    useEffect(() => {
        const verifyAdmin = async () => {
            ...
            // Verifikasi Pangkat & Akses High Admin
            const { data, error } = await supabase.from('users').select('pangkat, divisi, is_highadmin').eq('discord_id', parsed.discord_id).single();

            if (data) {
                const isAdmin = data.pangkat === 'JENDRAL' || data.divisi === 'PETINGGI' || data.is_highadmin === true;

                if (!isAdmin) {
                    toast.error("AKSES DITOLAK! Anda bukan High Command.");
    ...
    if (isChecking) return (
        <div className="min-h-screen bg-[#1e1e1e] flex flex-col items-center justify-center text-white font-mono font-black uppercase italic">
            <ShieldAlert size={64} className="mb-4 text-[#A78BFA] animate-pulse" />
            <div className="animate-pulse tracking-widest text-xl">Verifying Clearance...</div>
        </div>
    );
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
                ...
                <header className="flex justify-between items-center px-4 md:px-8 py-4 bg-white border-b-[6px] border-slate-950 z-40 shadow-[0px_6px_0px_#000]">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold uppercase tracking-widest opacity-60 italic">Admin Protocol Active</p>
                        </div>
                    </div>
                    <button onClick={() => router.push('/dashboard')} className="flex items-center gap-2 bg-[#FFD100] border-[3.5px] border-slate-950 px-4 py-2 rounded-xl text-xs font-black uppercase shadow-[4px_4px_0px_#000] active:translate-y-1 active:shadow-none transition-all">
                        <ArrowLeft size={16} /> <span className="hidden md:inline italic">Dashboard</span>
                    </button>
                </header>
                ...
                <main className="flex-1 overflow-y-auto p-4 md:p-8 pb-32">
                    <AnimatePresence mode="wait">
                        ...
                        {activeTab === 'laporan' && <SectionAdminLaporan key="laporan" />} {/* VIEW BARU */}
                        {activeTab === 'rekap' && <SectionAdminSystem key="rekap" />}
                    </AnimatePresence>
                </main>

                {/* BOTTOM NAVIGATION - 5 MENU FULL UPGRADED */}
                <nav className="fixed bottom-0 left-0 w-full bg-white border-t-[6px] border-slate-950 flex justify-around items-center py-4 pb-6 md:pb-4 z-[60] shadow-[0px_-6px_0px_rgba(0,0,0,1)]">
