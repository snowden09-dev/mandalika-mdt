...
import {
    Trash2, Eye, X, AlertOctagon, Shield, MapPin, Database, Loader2, Send, FileSpreadsheet, Target, UserX, PlusCircle, ChevronLeft, ChevronRight, Settings2, Filter, Calendar
} from 'lucide-react';
...
export default function SectionAdminPayroll() {
    ...
    // 🚀 STATE DATE RANGE PICKER UNTUK FILTER PERIODE REKAP
    ...
    const fetchData = async () => {
        setLoading(true);
        // Integrasi mengambil pengajuan gaji dari anggota
    ...
    useEffect(() => { setCurrentPage(1); }, [activeTab, startDateFilter, endDateFilter]);
    ...
    // 🚀 FILTERED DATA DENGAN DATE RANGE PICKER
    const filteredData = useMemo(() => {
        ...
        if (activeTab === 'REKAP') {
            ...
            if (startDateFilter && endDateFilter) {
                data = data.filter(r => {
                    const reqStart = format(getLocalSafeDate(r.tanggal_mulai), 'yyyy-MM-dd');
                    const reqEnd = format(getLocalSafeDate(r.tanggal_selesai), 'yyyy-MM-dd');
                    return reqStart >= startDateFilter && reqEnd <= endDateFilter;
                });
            }
            return data;
        }
    ...
    }, [augmentedRequests, activeTab, startDateFilter, endDateFilter]);
        ...
        return (
            <div className="flex justify-between items-center bg-slate-950 text-white p-4 rounded-xl border-[4px] border-black mt-6 shadow-[6px_6px_0px_#FFD100]">
                ...
                <div className="flex gap-2">
                    <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="bg-white text-black p-2 rounded-lg active:scale-95 disabled:opacity-30 transition-transform"><ChevronLeft size={16} strokeWidth={3} /></button>
                    <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="bg-white text-black p-2 rounded-lg active:scale-95 disabled:opacity-30 transition-transform"><ChevronRight size={16} strokeWidth={3} /></button>
                </div>
            </div>
        );
    };
        ...
        <div className="w-full max-w-7xl mx-auto space-y-6 font-mono pb-20 text-slate-950">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mb-8">
                ...
                <div className={`bg-[#FFD100] p-5 md:p-6 ${boxBorder} ${hardShadow} rounded-[20px] md:rounded-3xl`}>
                    <p className="text-[10px] font-black uppercase opacity-60">Pending Needs (Member Requests)</p>
            ...
            <div className={`bg-white ${boxBorder} ${hardShadow} p-4 md:p-6 rounded-xl md:rounded-2xl flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 relative z-10 mb-6`}>
                ...
                <div className="flex w-full lg:w-auto flex-col lg:flex-row items-center gap-3">
                    {/* 🚀 DATE RANGE PICKER FILTER UNTUK TAB REKAP */}
                    {activeTab === 'REKAP' && (
                        <div className="flex flex-wrap items-center gap-2 bg-slate-100 p-2 border-2 border-black rounded-xl w-full lg:w-auto">
                            ...
                            <div className="flex items-center gap-1 text-[10px] font-black uppercase">
                                ...
                                <input
                                    ...
                                    className="bg-white border-2 border-black px-2 py-1 rounded cursor-pointer outline-none"
                                />
                                <span className="opacity-60 ml-2">S/d:</span>
                                <input
                                    type="date"
                                    value={endDateFilter}
                                ...
                                {(startDateFilter || endDateFilter) && (
                                    <button 
                                        ...
                                        className="bg-red-500 text-white px-2 py-1 rounded ml-1 active:scale-95"
                                        title="Reset Filter"
                                    >
                                        <X size={12} />
                                    </button>
                    ...
                    <div className="flex flex-1 w-full lg:w-auto bg-slate-100 p-1.5 rounded-xl border-2 border-black gap-1 overflow-x-auto custom-scrollbar">
                        {['PENDING', 'NOT_SENT', 'PAID', 'REJECTED', 'REKAP'].map((t) => (
                            <button key={t} onClick={() => setActiveTab(t as any)} className={cn("px-3 md:px-4 py-2 rounded-lg text-[9px] md:text-[10px] font-black uppercase italic whitespace-nowrap flex items-center gap-2", activeTab === t ? "bg-[#00E676] border-2 border-black shadow-[2px_2px_0px_#000]" : "opacity-40 hover:bg-black/5")}>
                                ...
                                {t === 'PENDING' ? `PENDING (${requests.filter(r => r.status === 'PENDING').length})` : t.replace('_', ' ')}
                            </button>
                        ))}
                    </div>
                    {activeTab !== 'PENDING' && (
                        <button onClick={() => setDeleteModal({ show: true, type: 'ALL' })} className="bg-red-500 text-white p-2.5 rounded-xl border-2 border-black w-full lg:w-auto flex justify-center hover:scale-105 transition-all shadow-[2px_2px_0px_#000] active:translate-y-1"><Trash2 size={20} /></button>
                    )}
                </div>
            </div>
                            ...
                            <table className="w-full text-left border-collapse min-w-[1000px]">
                                ...
                                <tbody>
                                    {paginatedData.length === 0 ? (
                                        <tr><td colSpan={5} className="p-10 text-center font-black italic opacity-40 uppercase">Belum ada data rekap gaji pada rentang tanggal tersebut.</td></tr>
                                    ) : (
            ...
            {!loading && activeTab !== 'REKAP' && (
                paginatedData.length === 0 ? (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`bg-white ${boxBorder} ${hardShadow} rounded-[30px] p-10 md:p-20 flex flex-col items-center justify-center text-center mt-8`}>
                        ...
                        <p className="text-xs font-black uppercase opacity-50 mt-2 max-w-sm">Saat ini tidak ada pengajuan gaji anggota pada status <span className="text-blue-500">{activeTab.replace('_', ' ')}</span>.</p>
                    </motion.div>
                ) : (
                    <>
