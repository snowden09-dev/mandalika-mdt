...
import {
    ...
    BookOpen, FileText, Scale, Info, Book, Zap, Sun, Moon
} from 'lucide-react';
...
// 🚀 KOMPONEN DROPDOWN PROFIL DENGAN TOGGLE THEME
function ProfileDropdownMenu({
    ...
    const toggleTheme = () => {
        const newTheme = theme === 'NEO' ? 'CLEAN' : 'NEO';
        setTheme(newTheme);
            ...
            <AnimatePresence>
                {isProfileOpen && (
                    ...
                    >
                        ...
                        <button
                            onClick={toggleTheme}
                            className="flex items-center justify-between w-full p-3 hover:bg-slate-100 rounded-xl border-2 border-transparent transition-all group"
                        >
                            <div className="flex items-center gap-3">
                                <div className={`p-1.5 border-2 border-black rounded-lg transition-colors ${theme === 'NEO' ? 'bg-[#CCFF00]' : 'bg-red-500 text-white'}`}>
                                    {theme === 'NEO' ? <Zap size={16} /> : <Sun size={16} />}
                                </div>
                                <span className="font-black text-[10px] uppercase italic text-black">
                                    Style: {theme}
                                </span>
                            </div>
                            <div className="w-10 h-5 bg-slate-200 border-2 border-black rounded-full relative">
                                <motion.div
                                    animate={{ x: theme === 'NEO' ? 2 : 20 }}
                                    className="absolute top-0.5 w-3 h-3 bg-black rounded-full"
                                />
                            </div>
                        </button>

                        <div className="h-0.5 bg-black/10 my-1 mx-2" />
...
export default function PortalPage() {
    ...
    const [theme, setTheme] = useState<'NEO' | 'CLEAN'>('NEO'); // 🌓 Theme State
    ...
    useEffect(() => {
        ...
        const savedTheme = localStorage.getItem('mdt_theme') as 'NEO' | 'CLEAN';
        ...
        const checkUser = async () => {
            ...
            if (data) {
                ...
                setDbStatus({ is_admin: data.is_admin || data.is_highadmin, is_highadmin: data.is_highadmin });
                setRealtimeData({
    ...
    return (
        <div className={`flex min-h-screen font-mono overflow-hidden text-black transition-colors duration-500 ${theme === 'NEO' ? 'bg-[#E0E7FF]' : 'bg-slate-50'}`}>
            <Sidebar isOpen={isSidebarOpen} activeTab={activeTab} setActiveTab={setActiveTab} dbStatus={dbStatus} />

            <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
                <header className={`flex justify-between items-center px-6 py-4 bg-white border-b-[6px] border-black z-40 transition-shadow ${theme === 'NEO' ? 'shadow-[0_4px_0_0_rgba(0,0,0,1)]' : 'shadow-sm'}`}>
                ...
                <main className="flex-1 overflow-y-auto p-4 md:p-10 pb-32">
                    <AnimatePresence mode="wait">
                        {/* 🚀 Lemparkan Theme ke SectionHome */}
                        {activeTab === 'home' && (
                            <SectionHome
                                key={`home-${theme}`}
                                nickname={nickname}
                ...
                <nav className={`md:hidden fixed bottom-0 left-0 w-full bg-[#FFD100] border-t-[6px] border-black flex justify-around items-center py-4 pb-8 z-[60] ${theme === 'NEO' ? 'shadow-[0_-6px_0_0_rgba(0,0,0,1)]' : ''}`}>
