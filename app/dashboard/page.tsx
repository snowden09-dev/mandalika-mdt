...
// 🚀 KOMPONEN DROPDOWN PROFIL ULTRA-CLEAN MINIMALIST (TRAKTEER STYLE)
function ProfileDropdownMenu({ nickname, pangkat, image }: { nickname: string, pangkat: string, image: string }) {
    ...
    return (
        <div className="relative z-[999]" ref={dropdownRef}>
            ...
            >
                <div className="hidden md:flex flex-col items-end leading-tight font-sans">
                    <span className="text-xs font-semibold text-neutral-200 tracking-wide">{nickname}</span>
                    <span className="text-[11px] font-medium text-neutral-400 group-hover:text-red-500 transition-colors">{pangkat}</span>
                </div>

                {/* Avatar Bulat Bersih */}
                <div className="w-9 h-9 rounded-full border border-neutral-800 bg-[#1a1a1a] overflow-hidden relative flex-shrink-0 transition-all">
                    {image ? (
                        <NextImage src={image} alt="Profile" width={36} height={36} className="object-cover w-full h-full" />
                    ) : (
                        <User size={18} className="m-auto mt-2 text-neutral-400" />
                    )}
                    <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-red-500 border-2 border-[#151515] rounded-full"></span>
                </div>
            </button>
            ...
            <AnimatePresence>
                {isProfileOpen && (
                    <motion.div
                        ...
                        className="absolute right-0 top-full mt-2 w-52 bg-[#1a1a1a] border border-neutral-800/70 rounded-xl shadow-2xl p-1.5 flex flex-col gap-0.5 font-sans"
                    >
                        {/* Info Mobile */}
                        <div className="p-2.5 border-b border-neutral-800/60 mb-1 bg-[#121212]/40 rounded-lg md:hidden">
                            <p className="text-xs font-semibold text-neutral-200 truncate">{nickname}</p>
                            <p className="text-[11px] text-neutral-400 mt-0.5">{pangkat}</p>
                        </div>

                        <button onClick={() => router.push('/panduan')} className="flex items-center gap-3 p-2 hover:bg-neutral-800/50 rounded-lg transition-colors group w-full text-left">
                        ...
                        <button onClick={() => router.push('/sop')} className="flex items-center gap-3 p-2 hover:bg-neutral-800/50 rounded-lg transition-colors group w-full text-left">
                        ...
                        <button onClick={() => router.push('/uu')} className="flex items-center gap-3 p-2 hover:bg-neutral-800/50 rounded-lg transition-colors group w-full text-left">
                        ...
                        <button onClick={() => router.push('/about')} className="flex items-center gap-3 p-2 hover:bg-neutral-800/50 rounded-lg transition-colors group w-full text-left">
    ...
    return (
        <div className="flex min-h-screen bg-[#121212] font-sans overflow-hidden text-neutral-200 selection:bg-red-600/20">
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden relative bg-[#121212]">
                {/* HEADER (NAV BAR ATAS) */}
                <header className="flex justify-between items-center px-6 py-3.5 bg-[#151515] border-b border-neutral-800/80 z-40">
                    <div className="flex items-center gap-4">
                        <button 
                            onClick={() => setIsSidebarOpen(!isSidebarOpen)} 
                            className="hidden md:flex w-9 h-9 text-neutral-400 hover:text-white hover:bg-neutral-800/40 rounded-lg items-center justify-center transition-all"
                        >
                            {isSidebarOpen ? <ChevronLeft size={18} /> : <Menu size={18} />}
                        </button>
                        <h2 className="font-medium text-sm md:text-base text-neutral-100 tracking-wide">
                            {activeTab === 'home' && 'Feed / Personnel Terminal'}
                ...
                {/* BOTTOM NAVIGATION (MOBILE - MODEL PIL CAPSULE SEPERTI DI GAMBAR) */}
                <nav className="md:hidden fixed bottom-0 left-0 w-full bg-[#151515]/95 backdrop-blur-md border-t border-neutral-800/60 flex justify-around items-center py-2 pb-6 z-[60]">
                    <button 
                        onClick={() => setActiveTab('home')} 
                        className={`flex flex-col items-center gap-1 py-1.5 px-4 rounded-xl transition-all ${activeTab === 'home' ? 'text-red-500 bg-neutral-800/40' : 'text-neutral-500'}`}
                    >
                        <Home size={20} />
                    ...
                    <button 
                        ...
                        className={`flex flex-col items-center gap-1 py-1.5 px-4 rounded-xl transition-all ${activeTab === 'log' ? 'text-red-500 bg-neutral-800/40' : 'text-neutral-500'}`}
                    >
                        <History size={20} />
                    ...
                    <button 
                        ...
                        className={`flex flex-col items-center gap-1 py-1.5 px-4 rounded-xl transition-all ${activeTab === 'payroll' ? 'text-red-500 bg-neutral-800/40' : 'text-neutral-500'}`}
                    >
                        <Banknote size={20} />
                    ...
                    <button 
                        ...
                        className={`flex flex-col items-center gap-1 py-1.5 px-4 rounded-xl transition-all ${activeTab === 'handbook' ? 'text-red-500 bg-neutral-800/40' : 'text-neutral-500'}`}
                    >
                        <Book size={20} />
                    ...
                    {dbStatus.is_admin && (
                        <button 
                            ...
                            className="flex flex-col items-center gap-1 py-1.5 px-4 text-red-600/80 hover:text-red-500 transition-all"
                        >
                            <ShieldAlert size={20} />
